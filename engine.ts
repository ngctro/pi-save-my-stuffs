...
function restoreReadme(remote: string): string {
  const webUrl = remote.replace(/\.git$/, "");
  return `# pi configuration backup

Generated by [pi-save-my-stuffs](https://github.com/ngctro/pi-save-my-stuffs). It mirrors your pi agent directory: \`settings.json\`, \`extensions/\`, \`skills/\`, \`themes/\`, and \`prompts/\`.

Secrets are never backed up. \`auth.json\`, \`models-store.json\`, keys, and credential-looking files stay out.

## Restore on a new machine

1. Install pi, then authenticate GitHub so private remotes and git packages can install.

   \`\`\`
   gh auth login
   \`\`\`

2. Clone this backup and copy the mirrored paths back into place.

   \`\`\`
   git clone ${webUrl}.git
   mkdir -p ~/.pi/agent
   cp <backup-repo>/settings.json ~/.pi/agent/settings.json
   for dir in extensions skills themes prompts; do
     [ -d "<backup-repo>/$dir" ] && cp -r "<backup-repo>/$dir" ~/.pi/agent/
   done
   \`\`\`

3. Start pi. Packages listed in \`settings.json\` reinstall automatically on startup.

4. Recreate credentials with \`/login\` inside pi. They were never in this repo.

## Notes

- Every machine pushes to the same branch. The latest sync wins per file.
- This README is regenerated on every sync.
`;
}
...
async function syncOnce(opts: { agentDir: string; stateDir: string; hostname?: string }): Promise<SyncOutcome> {
  ...
  const committed = await convergeAndCommit(worktree, opts.agentDir, branch, hostname, resolveRemote(config.repo));
  ...
  if (push.code !== 0 && /reject|non-fast-forward/i.test(push.stderr)) {
    await convergeAndCommit(worktree, opts.agentDir, branch, hostname, resolveRemote(config.repo));
...
async function convergeAndCommit(
  ...
  fs.writeFileSync(path.join(worktree, "README.md"), restoreReadme(remote));
